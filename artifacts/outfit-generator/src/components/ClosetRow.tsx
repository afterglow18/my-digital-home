...
const BG_CENTER   = "hsl(45,33%,98%)";        // cream card for selected item
const SHADOW_CTR  = "0 4px 20px rgba(0,0,0,0.18), 0 1px 6px rgba(0,0,0,0.12)";
        ...
        >
          {items.slice(lo, hi + 1).map((item, localIdx) => {
            ...
            if (transitioning) {
              ...
              bg      = p > 0.5 ? `hsla(45,33%,98%,${((p - 0.5) * 2).toFixed(3)})` : "transparent";
              shadow  = p > 0.05
                ? `0 ${(4 * p).toFixed(1)}px ${(16 * p).toFixed(1)}px rgba(0,0,0,${(0.18 * p).toFixed(3)})`
                : "none";
            }
